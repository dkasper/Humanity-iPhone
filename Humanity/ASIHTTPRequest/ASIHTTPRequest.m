...
- (void)requestFinished
{
    ...
    [self performSelector:@selector(reportFinished) onThread:callingThread withObject:nil waitUntilDone:NO];
...
/* ALWAYS CALLED ON CALLERS THREAD! */
- (void)reportFailure
{
...
- (void)failWithError:(NSError *)theError
{
	...
	[failedRequest performSelector:@selector(reportFailure) onThread:callingThread withObject:nil waitUntilDone:NO];
...
{	
	...
	if ([self downloadComplete] && [self needsRedirect]) {
		...
		if (needToAskDelegateAboutRedirect) {
			...
			[self setRedirectURL:nil];
			[self performSelector:@selector(requestWillRedirectToURL) onThread:callingThread withObject:newURL waitUntilDone:NO];
...
+ (NSString *)defaultUserAgentString
{
	...
	if ([defaultUserAgentString length]) {
	...
	defaultUserAgentString = [[NSString alloc] initWithFormat:@"%@ %@ (%@; %@ %@; %@)", appName, appVersion, deviceName, OSName, OSVersion, locale];
...
+ (void)registerForNetworkReachabilityNotifications
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:JTVReachabilityChangedNotification object:nil];
}
