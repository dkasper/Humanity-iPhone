...
@synthesize accessToken = _accessToken,
        ...
        sessionDelegate = _sessionDelegate,
           ...
           fbDidOpenApp = _fbDidOpenApp,
          fbDidCloseApp = _fbDidCloseApp,
...
- (void)authorizeWithFBAppAuth:(BOOL)tryFBAppAuth
  ...
  /*
  HACK around a bug on facebook single sign-on. Seems old versions of the FB app don't pass 
  this 'sso_key' to the FB auth servers, and that including it in the auth URL fixes the problem.s
     
  */
 
    [params setValue:[[NSBundle mainBundle].infoDictionary objectForKey:@"CFBundleIdentifier"] forKey:@"sso_key"];
  ...
  _fbDidCloseApp = didOpenOtherApp;
...
- (void)authorize:(NSArray *)permissions delegate:(id<FBSessionDelegate>)delegate {
    [self authorize:permissions method:nil delegate:delegate];          
}


- (void)authorize:(NSArray *)permissions method:(NSString *)method delegate:(id<FBSessionDelegate>)delegate {
     [self authorize:permissions method:method delegate:delegate localAppId:nil];
}
 ...
 */
- (void)authorize:(NSArray *)permissions method:(NSString *) method delegate:(id<FBSessionDelegate>)delegate localAppId:(NSString *)localAppId {
  ...
  NSLog(@"authorize method:%@", method);
  
  if ([method isEqual:@"fb_app"]) {
         
  } else if ([method isEqual:@"web_view"]) {
      useApp = NO;
      useSafari = NO;
  } else if ([method isEqual:@"safari"]) {
      useApp = NO;
  }
  [self authorizeWithFBAppAuth:useApp safariAuth:useSafari];
}
